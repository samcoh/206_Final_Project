...
class Movie():
    def __init__(self, name, year, time, url, rating, genre, descrip, directors, num_directors, stars, num_stars, more_details_url, gross = "No Gross", weekend="No Opening Weekend Usa", budget="No Budget", cumulative = "No Cumulative Worldwide Gross"):
        ...
        if budget == "No Budget":
            ...
            bud = budget.split("\n")
            self.movie_budget = "".join(bud).replace(" ", "")
    ...
    for y in content:
        try:
            ...
            for x in c:
                ...
                #print(director)
                #print(type(director))
                number_of_directors = len(director.split(","))
                ...
                for actor in a:
                    ...
                    star = star + act
                stars = star[:-1]
                number_of_stars = len(stars.split(','))
                # stars= []
                # for actor in a:
                #     act = actor.find("a").text
                #     stars.append(act)
...
def update_movies_playing(theater_object):
    ...
    theater_address = theater_object.street_address
    ...
    statement = '''
        ...
        SET MoviesPlaying=?
        WHERE Name = '{}' AND StreetAddress = '{}'
    '''.format(theater_name, theater_address)
...
def budget_and_cumulativegross(theater_object):
    ...
    try:
        ...
        theater_name = theater_object.theater_name
        title = 'Movie Budget Compared to Cumulative Worldwide Gross \n for Movies Playing at {}'.format(theater_name)
        budget = []
        worldwide_gross = []
        MoviesShowing = []
        statement = '''
...
def minutes_of_movies(theater_object):
    ...
    try:
        ...
        if len(minutes) == 0:
            print("Data not avaliable to make this chart: Movie Length (in minutes) for Movies Playing")
            return []
        else:
...
def time_budget(theater_object):
    ...
    try:
        theater_streetaddress = theater_object.street_address
        theater_name = theater_object.theater_name
        title = 'Length of Movie (minutes) Compared to Movie Budget \n for Movies Playing at {}'.format(theater_name)
        budget = []
        minutes = []
        MoviesShowing = []
        statement = '''
        ...
        for x in movie_ids:
            ...
            for y in cur:
                ...
                if y[1]== None:
                    ...
                    MoviesShowing.append(y[2])
        if len(budget) == 0:
            print("Data not avaliable to make this chart: Movie Length (in minutes) Compared to Movie Budget for Movies Playing")
            return []
        elif len(minutes) == 0:
            print("Data not avaliable to make this chart: Movie Length (in minutes) Compared to Movie Budget for Movies Playing")
            return []
...
def OpeningWeekendUSA_compared_GrossUSA(theater_object):
    ...
    try:
        theater_streetaddress = theater_object.street_address
        theater_name = theater_object.theater_name
        title = 'Gross Profit Compared to Gross Profit During Opening Weekend in the USA \n for Movies Playing at {}'.format(theater_name)
        openingweekend = []
        gross = []
        MoviesShowing = []
        statement = '''
        ...
        for x in movie_ids:
            ...
            for y in cur:
                ...
                if y[1] == None:
                    ...
                    MoviesShowing.append(y[2])
        if len(gross) == 0:
            print("Data not avaliable to make this chart: Gross Profit Compared to Gross Profit During Opening Weekend in the USA for Movies Playing")
            return []
        elif len(openingweekend) == 0:
            print("Data not avaliable to make this chart: Gross Profit Compared to Gross Profit During Opening Weekend in the USA for Movies Playing")
            return []
        else:
            trace1 = go.Bar(
    ...
    while response != 'exit':
        ...
        if split_response[0]== "zip" and len(split_response[1]) == 5:
            try:
                ...
                print("No Data for the Command Entered.")
                response = input('Please type in a valid command (or "help" for more options): ')
                if response == "exit":
                    print("Goodbye!")
                continue
